...
Operations are `capabilities`, `subjects`, `workspaces`, `read`, and `update`. Discovery accepts `cursor` and `limit` (1–50); read requires `subject`; update requires `subject`, `expected_revision`, and the complete `access` value. The service injects `application` and the `contract_version` configured for that application (see [contract version 2](#contract-version-2)); callers cannot supply an actor. Subjects are bounded to 191 bytes, revisions to 128, and memberships to 100. Requests are limited to 64 KiB and streamed responses to 256 KiB.
